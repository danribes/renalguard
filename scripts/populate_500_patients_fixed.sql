-- ===============================================================
-- POPULATE 500 PATIENTS WITH COMPREHENSIVE CKD DATA (FIXED)
...
BEGIN
    RAISE NOTICE 'Starting population of 500 patients with unique names...';
    ...
    RAISE NOTICE 'Successfully populated 500 patients with unique gender-appropriate names!';
...
SELECT
    'Duplicate Name Check' as metric,
    COUNT(*) as duplicates
FROM (
...
-- Verify gender-appropriate names
SELECT
    'Male patients with male names' as metric,
    COUNT(*) as count
FROM patients
WHERE gender = 'male'
  AND first_name IN ('James', 'Robert', 'Michael', 'William', 'David', 'Richard', 'Joseph', 'Thomas', 'Charles', 'Christopher');

SELECT
    'Female patients with female names' as metric,
    COUNT(*) as count
FROM patients
WHERE gender = 'female'
...
SELECT 'Data population complete! All names are unique and gender-appropriate.' as status;
